...
import {
  ReloadRating,
  InsertRestaurants,
  Rlod_detail_rating_restaurants,
  LoadAvg_ratingRestaurants,
  GetRatingRestaurantsById,
  Remove,
...
const handleSubmitRating = async () => {
  ...
  try {
    ...
    const response = await InsertRestaurants(Datas)
  ...
  const loadAvg_rating = async () => {
    try {
      const avg_rating = await LoadAvg_ratingRestaurants(params.id);
  ...
  const loadDetailRating = async () => {
    try {
      const detail_rating_place = await Rlod_detail_rating_restaurants(
        params.id
      );
...
const RatingbyUser = isDetailRating.filter(item => item.user_id === user_id || item.rest_id === params.id);
        ...
        <section>
          {isData && (
            <>
                  ...
                  <div className="titles relative w-full flex-col rounded-xl overflow-hidden  group">
                    ...
                    <div className="price  flex flex-col md:flex-row lg:flex w-full p-4 gap-4  ">
                      <div className="title-header  items-center md:w-1/2 w-full">
                        ...
                        <p className="price capitalize w-full md:w-[70%]  md:text-xl text-white bg-gradient-to-r from-purple to-pink-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-purple200  font-medium rounded-lg text-sm px-2  py-2.5 text-center me-2 mb-2">
                          {"ເລີ່ມຕົ້ນ:  " + Number(isData.rest_price ).toLocaleString() + "  ກີບ"}
                        </p>
                      </div>
                    ...
                    <div className="price  flex-col w-full ">
                      <div className="font-semibold flex items-center justify-center p-2 pl-6">
                        <h3 className="font-semibold">ປະເພດເມນູ</h3>
                      </div>
                      <hr className=" h-2 " />
                      <div className="dec p-2 pl-8">
                        <article>{isData.rest_menu}</article>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="price  flex-col w-full mt-7 ">
                      <div className="font-semibold flex items-center justify-center p-2 pl-6">
                        <h3 className="font-semibold">ຄຳອທິບາຍເພີມເຕີມ</h3>
                      </div>
                      <hr className=" h-2 " />
                      <div className="dec p-2 pl-8 ">
                        <article className="md:text-2xl font-medium">{isData.rest_description}</article>
                      </div>
                    </div>

              {/* // todo image slider vevieq  */}

              <Carousel className="rounded-xl mt-4 overflow-hidden h-auto">
              ...
              <div className="Container-map mt-4">
                <h1 className="pl-7 md:text-2xl font-semibold ">{"ທີ່ຕັ້ງ "+" " +isData.rest_location}</h1>
